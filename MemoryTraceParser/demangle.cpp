...
#define MAX_TEMPLATE_SUBST  64
#define MAX_FUNC_NESTING    32
...
static void _source_name(struct mangle* mangle)
{
    ...
    if (mangle->valid) {
        ...
        char* tmpname = (char*)alloca(count + 1);
...
static void _pointer_to_member_type(struct mangle* mangle)
{
    ...
    if (expect(mangle, "M")) {
        ...
        char* classtype = (char*)alloca((len + 10) * sizeof(char));  /* add some space, because of characters concatenated */
...
static void _template_param(struct mangle* mangle)
{
    ...
    if (expect(mangle, "T")) {
        ...
        char* buffer = (char*)alloca((len + 10) * sizeof(char));
...
static void _ctor_dtor_name(struct mangle* mangle)
{
    ...
    if (mangle->valid) {
        ...
        char* cname = (char*)alloca((len + 1) * sizeof(char));
...
static void _function_encoding(struct mangle* mangle)
{
    ...
    if (has_return_type(mangle)) {
        char* mark = current_position(mangle);
        _type(mangle);
        type_string = (char*)alloca((strlen(mark) + 5) * sizeof(char));
